...
//3.长链剖分 DP 合并
template <class T = i64>
vector<T> depthMode(const vector<vector<Edge>>& g, int root = 1){
    int n = g.size() - 1;
    vector<int> len(n + 1, 1), son(n + 1);
    vector<int> at1(n + 1), at2(n + 1);
    vector<T> dp1(n + 5), dp2(4 * n + 5);
    vector<T> ans;
    //求最长链
    ...
    dfs1(dfs1, root, 0);
    //给轻儿子分配 DP 空间
    int ptr1 = len[root], ptr2 = 2 * len[root] + 5;
    at1[root] = 0; at2[root] = len[root];
    //DP
