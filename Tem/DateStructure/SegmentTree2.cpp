...
vector<ll> querylongest(int jobl,int jobr,int l,int r,int i){
    if(jobl<=l&&r<=jobr){
    ...
    down(i,ln,rn);
    if(jobr<=mid) return querylongest(jobl,jobr,l,mid,i<<1);
    if(jobl>mid) return querylongest(jobl,jobr,mid+1,r,i<<1|1);
    vector<ll> l3=querylongest(jobl,jobr,l,mid,i<<1);
    vector<ll> r3=querylongest(jobl,jobr,mid+1,r,i<<1|1);
