...
ll qrymax(int jobl,int jobr,int l,int r,int i){
    if(jobl<=l&&r<=jobr){
        return mx[i];
    }
    int mid=(l+r)>>1;
    ll ans=-INF;
    down(i,mid-l+1,r-mid);
    if(jobl<=mid) ans=max(ans,qrymax(jobl,jobr,l,mid,i<<1));
    if(jobr>mid) ans=max(ans,qrymax(jobl,jobr,mid+1,r,i<<1|1));
    return ans;
}
...
ll qryhistorymax(int jobl,int jobr,int l,int r,int i){
    if(jobl<=l&&r<=jobr){
        return maxHistory[i];
    }
    int mid=(l+r)>>1;
    ll ans=-INF;
    down(i,mid-l+1,r-mid);
    if(jobl<=mid) ans=max(ans,qryhistorymax(jobl,jobr,l,mid,i<<1));
    if(jobr>mid) ans=max(ans,qryhistorymax(jobl,jobr,mid+1,r,i<<1|1));
    return ans;
}
